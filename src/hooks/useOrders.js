...
export const useOrders = (addNotification, employees = [], isSyncingRef = { current: false }, saveTracker = null) => {
  ...
  const addOrder = useCallback(async () => {
    ...
    if (!orderForm.username.trim() || !orderForm.productName.trim() || !orderForm.description.trim()) {
      addNotification?.('⚠️ Username, Nama Product, dan Catatan harus diisi!', 'warning');
      return false;
    }
    ...
    setOrderForm(prev => ({
      username: '',
      platform: prev.platform, // Keep previous platform selection
      productName: '',
      description: '',
      quantity: '',
      status: ORDER_STATUS.PENDING
